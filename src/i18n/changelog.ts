    ...
    entries: [
      {
        ...
        date: 'Aug 5, 2026',
        items: [
          'Versión final — línea de producto cerrada; no se planea más desarrollo de funcionalidades',
          'Ventana de caducidad del build ~6 meses (183 días desde el empaquetado; cortex.beta.expirationDate)',
          'UX de límites de uso, turno sticky y visibilidad del historial al recargar (línea 0.2.2)',
        ],
      },
      {
        version: 'v0.2.2',
        date: 'Aug 3, 2026',
        items: [
          'Los límites de uso del proveedor ya no muestran “Reintentando en 398047s”: tiempo de reset claro, Cambiar modelo / facturación, y se corta la espera de días',
          'El turno activo queda sticky arriba mientras tools/thinking hacen scroll debajo (hasta idle)',
          'Tras recargar, el historial ya no queda oculto por la opacidad de sesión vacía mientras se vuelve a cargar',
          'Restore de arranque más ligero: solo la pestaña activa carga historial al inicio; evita base64 de imágenes y fetches duplicados que congelaban Cursor',
          'Historial de sesión paginado: el boot carga ~40 mensajes recientes; Cargar anteriores / scroll arriba pide la página previa a OpenCode',
          'Protección ante sesiones tóxicas: elimina patches enormes /.cortex/index del DiffSummary para que una pestaña no congele Cursor',
          'Rollback solo en mensajes de usuario: sin fork/return en pasos del asistente o herramientas',
          'Sin barra de copiar/reenviar bajo subagentes o tools — solo en prompts de usuario',
          'El toast de permisos ya no parpadea al reentregar el mismo request en el poll',
          'El scroll del chat se mantiene al aparecer diffs/permisos; el paint del timeline ya no salta al inicio',
          'Progreso en vivo del subagente en el timeline: la tarjeta task/subagente se mantiene abierta con actividad, pasos y tiempo (no solo el id de sesión hija)',
          'Un solo camino de timeline: actividad en vivo, avisos system/error/tool y hints de fork se remontan tras el paint (sin dual legacy/timeline)',
          'La pestaña activa restaura el historial tras el boot: el fetch de mensajes hace timeout en vez de colgarse; el restore recupera OpenCode y reintenta el tab enfocado',
          'El arranque recupera un OpenCode colgado: si el puerto acepta TCP pero el health no responde, Cortex lo mata y relanza en lugar de agotar el techo de boot',
          'El splash de arranque ya no parpadea en inglés: llega el idioma configurado y una ruleta breve de idiomas antes de los quips',
          'La sesión restaurada hace scroll al final tras el boot (el salto espera a que el chat sea visible)',
          'Diffs enriquecidos y progreso en vivo en el timeline: cards Keep/Undo otra vez; Thinking entre tools; el shimmer de “en curso” se limpia al idle',
          'Timeline restaurado: fork/reenviar/copiar al pasar el ratón, scroll al final al abrir un chat, y selector @ subagent en el composer',
          'El auto-aceptar permisos ya no deja el chat colgado: las respuestas usan rutas reales de OpenCode e ignoran HTML 200 de la SPA que dejaban bash en En curso',
          ...
          'Docs sincronizados con v0.2.0: tabla de settings, comandos de la Paleta, Servers/temas/notificaciones/composer; OpenCode CLI ≥ 1.18.0 en requisitos de instalación',
          'Página Comparar: tabla honesta Cortex vs Cursor vs Devin vs Zoo/Roo',
          'Docs: migrar desde Cursor / Copilot / Roo-Cline',
        ],
      },
      {
        version: 'v0.2.1',
    ...
    entries: [
      {
        ...
        date: 'Aug 5, 2026',
        items: [
          'Versão final — linha de produto encerrada; sem mais desenvolvimento de funcionalidades planeado',
          'Janela de expiração do build ~6 meses (183 dias desde o empacotamento; cortex.beta.expirationDate)',
          'UX de limites de utilização, turno sticky e visibilidade do histórico ao recarregar (linha 0.2.2)',
        ],
      },
      {
        version: 'v0.2.2',
        date: 'Aug 3, 2026',
        items: [
          'Limites de utilização do fornecedor já não mostram “A repetir em 398047s”: tempo de reset claro, Mudar modelo / faturação, e corta a espera de dias',
          'O turno ativo fica sticky no topo enquanto tools/thinking fazem scroll por baixo (até idle)',
          'Após recarregar, o histórico já não fica oculto pela opacidade de sessão vazia enquanto volta a carregar',
          'Restore de arranque mais leve: só o separador ativo carrega histórico no início; evita base64 de imagens e fetches duplicados que congelavam o Cursor',
          'Histórico de sessão paginado: o boot carrega ~40 mensagens recentes; Carregar anteriores / scroll para cima pede a página anterior ao OpenCode',
          'Proteção contra sessões tóxicas: remove patches enormes /.cortex/index do DiffSummary para uma aba não congelar o Cursor',
          'Rollback só em mensagens do utilizador: sem fork/return em passos do assistente ou ferramentas',
          'Sem barra de copiar/reenviar sob subagentes ou tools — só nos prompts do utilizador',
          'O toast de permissões já não pisca ao reentregar o mesmo pedido no poll',
          'O scroll do chat mantém-se ao aparecer diffs/permissões; o paint da timeline já não salta para o topo',
          'Progresso ao vivo do subagente na timeline: o cartão task/subagente fica aberto com atividade, passos e tempo (não só o id da sessão filha)',
          'Um único caminho de timeline: atividade ao vivo, avisos system/error/tool e hints de fork remount após o paint (sem dual legacy/timeline)',
          'O separador ativo restaura o histórico após o boot: o fetch de mensagens faz timeout em vez de pendurar; o restore recupera o OpenCode e tenta de novo o tab focado',
          'O arranque recupera um OpenCode pendurado: se a porta aceita TCP mas o health não responde, o Cortex mata e relança em vez de esgotar o teto de boot',
          'O splash de arranque já não pisca em inglês: chega o idioma configurado e uma roleta breve de idiomas antes dos quips',
          'A sessão restaurada faz scroll para o fim após o boot (o salto espera o chat ficar visível)',
          'Diffs enriquecidos + progresso ao vivo no timeline: cards Keep/Undo de novo; Thinking entre tools; shimmer “em curso” limpa no idle',
          'Timeline restaurado: fork/reenviar/copiar ao passar o rato, scroll para o fim ao abrir um chat, e seletor @ subagent no composer',
          'O auto-aceitar permissões já não deixa o chat preso: as respostas usam rotas reais do OpenCode e ignoram HTML 200 da SPA que deixavam o bash Em curso',
          ...
          'Docs sincronizados com v0.2.0: tabela de settings, comandos da Paleta, Servers/temas/notificações/composer; OpenCode CLI ≥ 1.18.0 nos requisitos de instalação',
          'Página Comparar: tabela honesta Cortex vs Cursor vs Devin vs Zoo/Roo',
          'Docs: migrar do Cursor / Copilot / Roo-Cline',
        ],
      },
      {
        version: 'v0.2.1',
    ...
    entries: [
      {
        ...
        date: 'Aug 5, 2026',
        items: [
          'Version finale — ligne produit fermée ; aucun développement de fonctionnalités supplémentaire prévu',
          'Fenêtre d’expiration du build ~6 mois (183 jours depuis le packaging ; cortex.beta.expirationDate)',
          'UX des limites d’usage, tour sticky et visibilité de l’historique au rechargement (ligne 0.2.2)',
        ],
      },
      {
        version: 'v0.2.2',
        date: 'Aug 3, 2026',
        items: [
          'Les limites d’usage fournisseur n’affichent plus « Nouvelle tentative dans 398047s » : délai de reset lisible, Changer de modèle / facturation, et arrêt de l’attente de plusieurs jours',
          'Le tour actif reste sticky en haut pendant que tools/thinking défilent en dessous (jusqu’à idle)',
          'Après rechargement, l’historique n’est plus masqué par l’opacité « session vide » pendant le rechargement',
          'Restore de démarrage plus léger : seul l’onglet actif charge l’historique au démarrage ; évite base64 d’images et fetchs en double qui figeaient Cursor',
          'Historique de session paginé : le boot charge ~40 messages récents ; Charger plus anciens / scroll haut récupère la page précédente via OpenCode',
          'Garde anti-session toxique : retire les patches énormes /.cortex/index du DiffSummary pour qu’un onglet ne fige plus Cursor',
          'Rollback uniquement sur les messages utilisateur : pas de fork/return sur les étapes assistant ou outils',
          'Pas de barre copier/renvoyer sous sous-agents ou tools — uniquement sur les prompts utilisateur',
          'Le toast de permission ne clignote plus quand le poll renvoie la même demande',
          'Le scroll du chat reste en place quand diffs/permissions apparaissent ; plus de saut en haut',
          'Progression live du sous-agent sur la timeline : la carte task/sous-agent reste ouverte avec activité, étapes et durée (plus seulement l’id de session enfant)',
          'Timeline unique : activité live, avis system/error/tool et hints de fork remontés après paint (plus de double chemin legacy/timeline)',
          'L’onglet actif restaure l’historique après le boot : le fetch des messages timeout au lieu de bloquer ; restore récupère OpenCode et réessaie l’onglet focalisé',
          'Le démarrage récupère un OpenCode bloqué : si le port accepte TCP mais le health ne répond pas, Cortex le tue et relance au lieu d’épuiser le plafond de boot',
          'Le splash de démarrage ne clignote plus en anglais : locale configurée + courte roulette de langues avant les quips',
          'La session restaurée défile jusqu’en bas après le boot (le saut attend que le chat soit visible)',
          'Diffs enrichis + progression live sur la timeline : cartes Keep/Undo ; Thinking entre les tools ; shimmer « en cours » effacé à l’idle',
          'Timeline restauré : fork/renvoi/copie au survol, scroll en bas à l’ouverture d’un chat, et sélecteur @ subagent dans le composer',
          'L’auto-acceptation des permissions ne bloque plus le chat : les réponses utilisent les vraies routes OpenCode et ignorent les HTML 200 SPA qui laissaient bash en En cours',
          ...
          'Docs synchronisées avec v0.2.0 : table des settings, commandes de la Palette, Servers/thèmes/notifications/composer ; OpenCode CLI ≥ 1.18.0 dans les prérequis d’installation',
          'Page Comparer : tableau honnête Cortex vs Cursor vs Devin vs Zoo/Roo',
          'Docs : migrer depuis Cursor / Copilot / Roo-Cline',
        ],
      },
      {
        version: 'v0.2.1',
    ...
    entries: [
      {
        ...
        date: 'Aug 5, 2026',
        items: [
          'Finale Version — Produktlinie geschlossen; keine weitere Feature-Entwicklung geplant',
          'Build-Ablauffrist ~6 Monate (183 Tage ab Packaging; cortex.beta.expirationDate)',
          'Nutzungslimit-UX, sticky Turn und Historien-Sichtbarkeit nach Reload (0.2.2-Linie)',
        ],
      },
      {
        version: 'v0.2.2',
        date: 'Aug 3, 2026',
        items: [
          'Anbieter-Nutzungslimits zeigen nicht mehr „Wiederholung in 398047s“: klare Reset-Zeit, Modell wechseln / Abrechnung, und kein mehrtägiges Warten',
          'Aktiver Turn bleibt oben sticky, während Tools/Thinking darunter scrollen (bis idle)',
          'Nach Reload bleibt die Historie nicht mehr hinter leerer-Session-Opacity verborgen, während sie neu geladen wird',
          'Leichterer Boot-Restore: nur der aktive Tab lädt Historie beim Start; kein Image-Base64 und keine Doppel-Fetches, die Cursor einfrieren ließen',
          'Paginierte Sitzungshistorie: Boot lädt die neuesten ~40 Nachrichten; Ältere laden / Scroll nach oben holt die vorherige OpenCode-Seite',
          'Schutz vor toxischen Sessions: riesige /.cortex/index DiffSummary-Patches werden entfernt, damit ein Tab Cursor nicht einfriert',
          'Rollback nur bei Benutzernachrichten: kein Fork/Return bei Assistenten- oder Tool-Schritten',
          'Keine Kopieren/Erneut-senden-Leiste unter Subagenten oder Tools — nur bei Benutzerprompts',
          'Permission-Toast flackert nicht mehr bei Poll-Wiederzustellung derselben Anfrage',
          'Chat-Scroll bleibt bei Diffs/Permissions; Timeline-Paint springt nicht mehr nach oben',
          'Live-Fortschritt des Subagents in der Timeline: Task/Subagent-Karte bleibt offen mit Aktivität, Schritten und Zeit (nicht nur die Kind-Session-ID)',
          'Ein Timeline-Pfad: Live-Aktivität, System/Error/Tool-Hinweise und Fork-Hints remounten nach Paint (kein Dual Legacy/Timeline)',
          'Aktiver Tab stellt die Sitzungshistorie nach dem Boot wieder her: Message-Fetch timeoutet statt zu hängen; Restore holt OpenCode zurück und retryt den Fokus-Tab',
          'Boot holt einen hängenden OpenCode zurück: akzeptiert der Port TCP ohne Health, killt und startet Cortex neu statt den Boot-Ceiling zu überschreiten',
          'Boot-Splash blitzt nicht mehr auf Englisch: konfigurierte Locale + kurze Sprach-Roulette vor den Quips',
          'Wiederhergestellte Session scrollt nach dem Boot ans Ende (Sprung wartet bis das Chat-Panel sichtbar ist)',
          'Angereicherte Diffs + Live-Fortschritt in der Timeline: Keep/Undo-Karten; Thinking zwischen Tools; hängendes „running“-Shimmer endet im Idle',
          'Timeline wiederhergestellt: Fork/Erneut senden/Kopieren bei Hover, Scroll ans Ende beim Öffnen eines Chats, und @-Subagent-Auswahl im Composer',
          'Auto-Accept von Berechtigungen hängt den Chat nicht mehr: Antworten nutzen echte OpenCode-Routen und ignorieren SPA-HTML-200, die bash bei In Bearbeitung festhielten',
          ...
          'Docs mit v0.2.0 synchronisiert: Settings-Tabelle, Befehlspalette, Servers/Themes/Benachrichtigungen/Composer; OpenCode CLI ≥ 1.18.0 in den Install-Anforderungen',
          'Vergleichsseite: ehrliche Tabelle Cortex vs Cursor vs Devin vs Zoo/Roo',
          'Docs: Migration von Cursor / Copilot / Roo-Cline',
        ],
      },
      {
        version: 'v0.2.1',
    ...
    entries: [
      {
        ...
        items: [
          '最终版本 — 产品线关闭；不再计划功能开发',
          '构建有效期约 6 个月（自打包日起 183 天；cortex.beta.expirationDate）',
          '用量上限 UX、粘性回合、重载后历史可见性（0.2.2 线）',
        ],
      },
      {
        version: 'v0.2.2',
        date: 'Aug 3, 2026',
        items: [
          '供应商用量上限不再显示“398047 秒后重试”：可读重置时间、更换模型/账单，并停止数天空等',
          '进行中的回合在顶部 sticky 固定，下方可滚动 tools/thinking（直至 idle）',
          '重新加载时不再用空会话透明度把历史藏起来，避免看起来像消息丢了',
          '启动恢复更轻：仅活动标签在启动时加载历史；跳过图片 base64 与重复请求，避免 Cursor 卡死',
          '会话历史分页：启动只加载约 40 条最新消息；加载更早 / 上滑向 OpenCode 请求上一页',
          '有毒会话防护：剥离巨大的 /.cortex/index DiffSummary 补丁，避免单个标签卡死 Cursor',
          '仅可回滚到用户消息：助手/工具步骤不再提供 fork/return',
          '子代理/工具卡片下不再显示复制/重发栏 — 仅在用户提示上',
          '同一权限请求被 poll 重复投递时，通知不再闪烁',
          '出现 diffs/权限时聊天滚动位置保持；时间线重绘不再跳到顶部',
          '时间线上实时显示子代理进度：task/subagent 卡片保持展开，含活动、步骤与耗时（不再只有子会话 id）',
          '统一时间线路径：实时活动、system/error/tool 通知与 fork 提示在重绘后重新挂载（无 legacy/timeline 双路径）',
          '启动后活动标签会恢复会话历史：拉取消息超时而非挂起；restore 会回收 OpenCode 并重试焦点标签',
          '启动会回收卡住的 OpenCode：端口能 TCP 但 health 超时则杀掉并重启，而不是卡死在 boot 上限',
          '启动闪屏不再先闪英文：内嵌已配置语言，并在俏皮话前短转语言轮盘',
          '恢复的会话在启动后滚到底（等到聊天面板可见再跳转）',
          '时间线恢复富化 diff 与实时进度：Keep/Undo 迷你卡片、工具步骤间 Thinking、idle 清除卡住的 running 闪烁',
          '时间线恢复：悬停 fork/重发/复制、打开聊天滚到底、composer 中的 @ subagent 选择器',
          '自动接受权限不再卡住聊天：回复走真实 OpenCode 路由，并忽略会让 bash 停在进行中的 SPA HTML 200',
          '工具卡住看门狗在等待问题卡片或 bash 权限时不再中止',
          '性能/质量的硬件适配提示：检测内存/CPU，显示本机是否适合质量（MiniLM 约150MB），并在紧张机器上限制嵌入并发',
          '文档与 v0.2.0 同步：设置表、命令面板、Servers/主题/通知/composer；安装要求中注明 OpenCode CLI ≥ 1.18.0',
          '对比页：诚实的 Cortex vs Cursor vs Devin vs Zoo/Roo 对照表',
          '文档：从 Cursor / Copilot / Roo-Cline 迁移',
        ],
      },
      {
        version: 'v0.2.1',
